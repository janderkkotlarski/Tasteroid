...
void show_nament(sf::RenderWindow& window, const name_entity& nament)
{
	...
	std::cout << nament.nament_text_.getString().toAnsiString() << "\n";
	
	const sf::Color color{nament.nament_text_.getColor()};
	
	std::cout << "[" << static_cast<int>(color.r)
			<< "," << static_cast<int>(color.g)
			<< "," << static_cast<int>(color.b) << "]" << "\n";
			
	const int char_size{static_cast<int>(nament.nament_text_.getCharacterSize())};
	
	std::cout << char_size << "\n";
	
	const sf::Vector2f posit{nament.nament_text_.getPosition()};
	
	std::cout << "[" << posit.x << "," << posit.y << "]" << "\n";
	
	const sf::Vector2f origin{nament.nament_text_.getOrigin()};
	
	std::cout << "[" << origin.x << "," << origin.y << "]" << "\n";
	
	const sf::FloatRect bounds{nament.nament_text_.getLocalBounds()};
	...
	rectent_circlent_values_(values)
{
	...
	rectent_rect_.setOrigin(rectent_rect_origin_);
	
	rectent_rect_.setPosition(rectent_rect_posit_);
	
	rectent_rect_.setFillColor(rectent_gold_);
	...
	const float norm_radius{0.05f*rectent_dimens_.y};
	
	std::cout << norm_radius << "\n";
	
	const sf::Vector2f circle_posit{0.25f*rectent_dimens_.y, 0.25f*rectent_dimens_.y};
	
	
	const sf::Vector2f circlent_posit{rectent_rect_posit_ + circle_posit - rectent_rect_origin_};
...
void show_rectent(sf::RenderWindow& window, const rectangle_entity& rectent)
{
	...
	if (current_circlent < circlent_number)
	{
			
		std::cout << current_circlent << "\n";	
		
		// show_circlent(window, rectent.rectent_circlent_);
...
int main()
{
	...
	const std::string program_name{"Texts V0.10"};	
	assert(program_name != "");
	...
	const sf::Vector2f rectent_sizes{0.25f*window_sizes};
	...
	rc_names.push_back("Tsubaki Shinra");
	
	std::vector <float> rc_values;
	
	rc_values.push_back(300.0f);
